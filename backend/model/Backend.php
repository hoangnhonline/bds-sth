...
class Backend {
    ...
    function countObject($object_type, $status = -1, $user_id=-1){
        $arr = array();
        $sql = "SELECT * FROM objects WHERE status = $status AND object_type = $object_type AND (user_id = $user_id OR $user_id = $user_id) ";
        $rs = mysql_query($sql) or die(mysql_error());
    ...
    /*function getListContractDenHan($user_id = -1){
        ...
        AND DAY( DATE_ADD( NOW( ) , INTERVAL 3 DAY )) >= DAY(start_date) AND contract.status = 1 AND (contract.user_id = $user_id OR $user_id = -1)
        ORDER BY start_date
        ";
        $rs = mysql_query($sql) or die(mysql_error());
...
/*
    function getListContractSapHetHan($user_id = -1){
        $arr = array();
        $sql = "SELECT contract.*, objects.name as name, objects.house_id as house_id  FROM `contract`, objects WHERE objects.id = contract.object_id 
        AND DATE( DATE_ADD( NOW( ) , INTERVAL 1 MONTH )) >= DATE(end_date) AND contract.status = 1 AND (contract.user_id = $user_id OR $user_id = -1)
        ORDER BY end_date
        ";
        $rs = mysql_query($sql) or die(mysql_error());
        while ($row = mysql_fetch_assoc($rs)) {
